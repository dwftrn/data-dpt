...
type Props = {
  ...
  closeDialog(): void
  onLoadinChange(loading: boolean): void
}
...
const VoteFormDialogContent = ({ data, pemilu, closeDialog, onLoadinChange }: Props) => {
  ...
  const { data: detail } = useFetchVoteDetail({ id: data.id_suara })
  ...
  const onSubmit = (values: FormType) => {
    ...
    if (+values.sah !== data.sah) {
      params.sah = +values.sah
      hasDataChanges = true
    }

    if (+values.tidak_sah !== data.tidak_sah) {
      params.tidak_sah = +values.tidak_sah
      hasDataChanges = true
    }
...
export default VoteFormDialogContent
